// code borrowed from: https://gist.github.com/emredesu/af597de14a4377e1ecf96b6f7b6cc506, slightly modified by me (yeah, but this is not the best script, it is hardcoded :/)
...
#nullable enable
    void InputScroll()
    {
        if(m_Selectables.Count > 0)
        {
            ...
            {
                if(Gamepad.current.dpad.up.isPressed || Gamepad.current.dpad.down.isPressed || Gamepad.current.rightStick.IsActuated() || Gamepad.current.leftStick.IsActuated())
                {
                    ScrollToSelected(false);
                }
            }
